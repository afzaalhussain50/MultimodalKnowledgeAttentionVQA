...
class OkvqaTrainDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        with open('../okvqa/question_raw_train_with_graphs20.json', 'r') as f:
            qa_raw = json.load(f)

        print('loading image_feature.npz')
        img_data = np.load('../okvqa/train_image_feature20.npz')
        ...
        sem_data = np.load('../okvqa/semantic_graph_train_feature20.npz', allow_pickle=True)
        ...
        # my changes in code (, allow_pickle=True added)
        fact_data = np.load('../okvqa/fact_graph_train_feature20.npz', allow_pickle=True)
