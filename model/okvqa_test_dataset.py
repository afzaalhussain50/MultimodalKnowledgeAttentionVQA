...
class OkvqaTestDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        print('loading dataset_opendomain_q_raw20.json')
        with open('../okvqa/question_raw_val_with_graphs20.json', 'r') as f:
            qa_raw = json.load(f)



        print('loading semantic_graph_feature20.npz')
        sem_data = np.load('../okvqa/semantic_graph_val_feature20.npz', allow_pickle=True)
        ...
        img_data = np.load('../okvqa/val_image_feature20.npz', allow_pickle=True)
        self.image_features = img_data['image_features']
        self.image_relations = img_data['image_relations']

        print('loading fact_graph_feature20.npz')
        fact_data = np.load('../okvqa/fact_graph_val_feature20.npz', allow_pickle=True)
    ...
    def sliced_data(self, slice_length):
        sliced_data = slice_length

        self.qids = self.qids[:sliced_data]
        self.questions = self.questions[:sliced_data]
        self.question_lengths = self.question_lengths[:sliced_data]

        self.fact_num_nodes = self.fact_num_nodes[:sliced_data]
        self.fact_e1ids_list = self.fact_e1ids_list[:sliced_data]
        self.fact_e2ids_list = self.fact_e2ids_list[:sliced_data]
        self.fact_answer_ids = self.fact_answer_ids[:sliced_data]
        self.fact_graph_node_features = self.fact_graph_node_features[:sliced_data]

        self.semantic_num_nodes = self.semantic_num_nodes[:sliced_data]
        self.semantic_e1ids_list = self.semantic_e1ids_list[:sliced_data]
        self.semantic_e2ids_list = self.semantic_e2ids_list[:sliced_data]
        self.semantic_graph_node_features = self.semantic_graph_node_features[:sliced_data]
        self.semantic_graph_edge_features = self.semantic_graph_edge_features[:sliced_data]

        self.image_features = self.image_features[:sliced_data]
        self.image_relations = self.image_relations[:sliced_data]
        self.image_ids = self.image_ids[:sliced_data]